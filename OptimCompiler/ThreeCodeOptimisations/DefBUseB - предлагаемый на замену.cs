    ...
    class DefUseBlocks 
    {
        ...
        private void MakeSets()
        {
            foreach(var block in Graph.blocks)
            {
                ...
                {
                    if (cmd.arg1 != null && cmd.arg1 is ThreeAddressStringValue && !cmd.arg1.ToString().StartsWith("temp_") && !DefB.Contains(cmd.arg1.ToString()))
                        UseB.Add(cmd.arg1.ToString());
                    if (cmd.arg2 != null && cmd.arg2 is ThreeAddressStringValue && !cmd.arg2.ToString().StartsWith("temp_") && !DefB.Contains(cmd.arg2.ToString()))
                        UseB.Add(cmd.arg2.ToString());
                    if (!cmd.result.ToString().StartsWith("temp_"))
                        DefB.Add(cmd.result);
                }
            }
        }
